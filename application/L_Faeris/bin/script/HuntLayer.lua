...
function HuntLayer:New()
	...
	layer.data={
		...
		onTouchesBegin=HuntLayer.onTouchesBegin,
		onTouchesMove=HuntLayer.onTouchesMove,
		onTouchesEnd=HuntLayer.onTouchesEnd,
	}
...
function HuntLayer:onTouchesBegin(num,event)
	f_utillog("touches begin")
	f_utillog("num=%d",num);
	for i=0,num-1 do 
		f_utillog("i=%d,id=%d,x=%f,y=%f",i,event[i].id,event[i].x,event[i].y)
	end


end

function HuntLayer:onTouchesMove(num,event)
	f_utillog("touches move")
	f_utillog("num=%d",num);
...
function HuntLayer:onTouchesEnd(num,event)
	f_utillog("touches end")
	f_utillog("num=%d",num);
