...
local function LayerBK_Create()
	...
	layerBK:setSortMode(Layer2D.SORT_ORDER_Z)
	local quadBK = Quad2D:create("textures/game/Bkgnd.png", Rect2D(0, 0, 480, 800))
	quadBK:setPosition(0, 0)
	
	local quadFuncArea = Quad2D:create("textures/game/functionArea.png")
	local quadGameArea = Quad2D:create("textures/game/gameArea.png")
	local quadNextBlockArea = Quad2D:create("textures/game/nextBlockArea.png")
	local quadHScrollBar = Quad2D:create("textures/game/HScrollBar.png")
	quadFuncArea:setPosition(403, 365)
	quadFuncArea:setZorder(1)
	quadGameArea:setPosition(167, 419)
	quadGameArea:setZorder(1)
	quadNextBlockArea:setPosition(403, 673)
	quadNextBlockArea:setZorder(1)
	quadHScrollBar:setPosition(169, 62)
	quadHScrollBar:setZorder(1)
	layerBK:add(quadFuncArea)
	layerBK:add(quadGameArea)
	layerBK:add(quadNextBlockArea)
	layerBK:add(quadHScrollBar)
	layerBK.data.quadFuncArea = quadFuncArea
	layerBK.data.quadGameArea = quadGameArea
	layerBK.data.quadNextBlockArea = quadNextBlockArea
	layerBK.data.quadHScrollBar = quadHScrollBar
...
local function LayerUI_Create()
	...
	AddButton(layerUI, 410, 772, 1, "textures/game/backOff.png", "textures/game/backOn.png", Rect2D(-64, -24, 128, 48))
	AddButton(layerUI, 404, 70, 2, "textures/game/changeOff.png", "textures/game/changeOn.png", Rect2D(-60, -60, 120, 120))
	AddButton(layerUI, 29, 769, 3, "textures/game/soundOn.png", "textures/game/soundOff.png", Rect2D(-21, -21, 42, 42))
	AddButton(layerUI, 90, 769, 4, "textures/game/musicOn.png", "textures/game/musicOff.png", Rect2D(-21, -21, 42, 42))
	
	layerUI.data.onTouchBegin = Press_OnTouchBegin
 	layerUI.data.onTouchMove = Press_OnTouchMove
 
 --[[ -- 调整坐标时用 test
	layerUI.data.onTouchMove = function (selfLayer, x, y)
		PrintCoordTest(selfLayer, x, y)
		x, y = selfLayer:toLayerCoord(x, y)
		selfLayer.data.quadHScrollBar:setPosition(x, y)
	end	
--]]
...
function ScenePlay_New()
	...
	local layerPlay = LayerPlay_Create()
	local layerUI = LayerUI_Create()
	
	scene:push(layerBK)
	scene:push(layerPlay)
	scene:push(layerUI)
	return scene
end
