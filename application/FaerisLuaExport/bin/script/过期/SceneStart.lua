...
local function StartLayer_SelectTouchEnd(self, x, y)
	...
	if focus == data.play then 
		data.nextScene = ScenePlay_New() -- ?在StartLayer_ExitUpdate(self,dt)中
...
local function StartLayer_StartUpdate(self,dt)
	...
	if total_frame - 1 == cur_frame then -- 帧弄完则换状态
...
local function StartLayer_CreateData() -- create data
	local back_ground = Sprite2D:create("sprites/start.fst")
	local play = Quad2D:create("textures/play.png", 122, 43)
	play:setPosition(480, 240) -- the play button's position
	local quit = Quad2D:create("textures/quit.png", 122, 43)
	quit:setPosition(480, 180) -- the quit button's position
	local setting = Quad2D:create("textures/settings.png", 270, 46)
	setting:setPosition(480, 120) -- the setting button's position
...
local function StartLayer_OnEnter(self) -- key
	local data=self.data
	data.focus=nil
	data.nextScene=nil
	...
	util.changeCallBack(self,StartLayer_mStartCallBack) -- key
end


local function StartLayer_SetFocus(self,ob) -- 就是点到按钮之后按钮放大
	local data=self.data
	if data.focus then 
...
local function StartLayer_New() -- 3 ///////////////////////////////////////////////
	local layer = Layer2D:create()
	layer.data = StartLayer_CreateData() -- ? updata 在哪？
	layer.onEnter = StartLayer_OnEnter -- key
...
local function SceneStart_CreateData() -- 2
	return { 
		-- attribute 
		layer = StartLayer_New(), -- 3
		-- call back 
		onEnter = SceneStart_OnEnter, -- ? wm: callback ?
		onExit = SceneStart_OnExit 
	} 
end

function SceneStart_New() -- 1
	local scene = Scene:create()
	scene.data = SceneStart_CreateData() -- 2
	return scene; -- director:run(start)
end
