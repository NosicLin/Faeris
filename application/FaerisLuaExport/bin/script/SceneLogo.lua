...
local function LayerLogo_OnEnter(selfLayer)
	print("run LayerLogo_OnEnter(selfLayer)")
	selfLayer.data.spriteLogo:setAnimation("default")
	selfLayer.data.spriteLogo:startAnimation(Sprite2D.ANIM_END)
end

local function LayerLogo_OnUpdate(selfLayer, dt)
	selfLayer:update(dt)
	local totalFrame = selfLayer.data.spriteLogo:getTotalFrame()
	local curFrame = selfLayer.data.spriteLogo:getCurFrame()
	
	if curFrame == totalFrame - 1 then
		-- 跳转到下一个scene
		print("change to next scene")
		local director = share:director()
		director:push()
		director:run(SceneStart_New())
	end
end

function SceneLogo_New()
	print("run SceneLogo_New()")
	local scene = Scene:create()
	local layer = Layer2D:create()
	local spriteLogo = Sprite2D:create("sprites/start.fst")
	layer:add(spriteLogo)
	layer.data = {}
	layer:setViewArea(0, 0, 480, 800)
	layer.data.spriteLogo = spriteLogo
	LayerLogo_OnEnter(layer)
	layer.data.onUpdate = LayerLogo_OnUpdate
	scene:push(layer)
