...
local function SceneStartChangeSceneTest(selfLayer, x, y)
	print("run SceneStartChangeSceneTest(selfLayer, x, y)")
	x, y = selfLayer:toLayerCoord(x, y)
	if selfLayer.data.quadSupport:hit2D(x, y) then
...
local function PrintCoordTest(selfLayer, x, y)
	SceneStartChangeSceneTest(selfLayer, x, y)
	x, y = selfLayer:toLayerCoord(x, y)
	print("point(" .. x .. "," .. y .. ")")
	return false
end
function SceneStart_New()
	...
	local spriteBK = Sprite2D:create("sprites/start.fst") -- 俄罗斯方块下落
	local quadBK = Quad2D:create("textures/Bkgnd.png", Rect2D(0, 0, 480, 800))
	local quadTitle = Quad2D:create("textures/Bkgnd_Main.png",Rect2D(0, 0, 480, 800))
	quadBK:setPosition(0, 0)
	quadTitle:setPosition(0, 0)
	layerBK:setViewArea(0, 0, 480, 800)
	layerBK.data = {}
	layerBK.data.quadBK = quadBK
	layerBK.data.quadTitle = quadTitle
	layerBK.data.spriteBK = spriteBK
	layerBK:add(spriteBK)
	layerBK:add(quadBK)
	layerBK:add(quadTitle)
	
	-- UI层
	local layerUI = Layer2D:create()
	layerUI:setTouchEnabled(true)
	local quadNewGame = Quad2D:create("textures/newGameButton.png")
	local quadContinue = Quad2D:create("textures/continueGameButton.png")
	local quadAchieve = Quad2D:create("textures/achivementButton.png")
	local quadSupport = Quad2D:create("textures/supportUs.png")
	local quadAbout = Quad2D:create("textures/aboutButton.png")
	local quadSignIn = Quad2D:create("textures/signInButton.png")
	quadNewGame:setPosition(373, 514)
	quadContinue:setPosition(373, 426)
	quadAchieve:setPosition(373, 346)
	quadSupport:setPosition(373, 282)
	quadAbout:setPosition(373, 190)
	quadSignIn:setPosition(387, 50)
	layerUI:add(quadNewGame)
	layerUI:add(quadContinue)
	layerUI:add(quadAchieve)
	layerUI:add(quadSupport)
	layerUI:add(quadAbout)
	layerUI:add(quadSignIn)
	layerUI:setViewArea(0, 0, 480, 800)
	layerUI.data = {}
	layerUI.data.quadNewGame = quadNewGame
	layerUI.data.quadContinue = quadContinue
	layerUI.data.quadAchieve = quadAchieve
	layerUI.data.quadSupport = quadSupport
	layerUI.data.quadAbout = quadAbout
	layerUI.data.quadSignIn = quadSignIn
	layerUI.data.onTouchBegin = PrintCoordTest
	scene:push(layerBK)
	scene:push(layerUI)
	return scene
end
