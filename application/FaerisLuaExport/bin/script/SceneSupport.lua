...
local function SceneSupporChangeSceneTest(selfLayer, x, y)
	x, y = selfLayer:toLayerCoord(x, y)
	print("point(" .. x .. "," .. y .. ")")
	if selfLayer.data.quadBack:hit2D(x, y) then
...
function SceneSupport_New()
	local scene = Scene:create()
	
	local layerBK = Layer2D:create()
	layerBK:setViewArea(0, 0, 480, 800)
	layerBK:setTouchEnabled(true)
	local quadBK = Quad2D:create("textures/Bkgnd_Support.png", Rect2D(0, 0, 480, 800))
	local quadBack = Quad2D:create("textures/backButton.png")
	quadBK:setPosition(0, 0)
	quadBack:setPosition(384, 58)
	layerBK:add(quadBK)
	layerBK.data = {}
	layerBK.data.quadBK = quadBK
	layerBK.data.quadBack = quadBack
	layerBK.data.onTouchBegin = SceneSupporChangeSceneTest
	scene:push(layerBK)
	return scene
end
